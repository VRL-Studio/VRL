...
public class Scope2Code {
	...
	public static String getCode(CompilationUnitDeclaration scope) {
		...
		CompilationUnitRenderer renderer = new CompilationUnitRenderer(
				new ClassDeclarationRenderer(new MethodDeclarationRenderer(
	...
	public static String getCode(ClassDeclaration scope) {
		...
		ClassDeclarationRenderer renderer = new ClassDeclarationRenderer(
				new MethodDeclarationRenderer(new InvocationCodeRenderer()));
	...
	public static String getCode(MethodDeclaration scope) {
		...
		MethodDeclarationRenderer renderer = new MethodDeclarationRenderer(
				new InvocationCodeRenderer());
...
final class Utils {
	...
	static void renderComments(Set<Comment> rendered,
			List<? extends CodeEntity> entities, Scope e, CodeBuilder cb,
			RenderEventListener rel) {
		...
		for (CodeEntity ce : entities) {
			...
			for (Comment comment : e.getComments()) {
				...
				if (comment.getRange().getEnd().getCharIndex() < ce.getRange()
						.getBegin().getCharIndex()) {
...
class InvocationCodeRenderer implements CodeRenderer<Invocation> {
	...
	private void render(Invocation i, CodeBuilder cb, boolean inParam) {
		...
		if (i.isConstructor()) {
			cb.append("new ").append(i.getVariableName()).append("(");
			renderArguments(i, cb);
			cb.append(")");

		} else if (i instanceof DeclarationInvocation) {
			DeclarationInvocation decl = (DeclarationInvocation) i;
			cb.append(decl.getDeclaredVariable().getType().getClassNameAsCode())
					.append(" ").append(decl.getDeclaredVariable().getName());
			...
			boolean letArgNeedsParantheses = operatorInvocation
					.getLeftArgument().getArgType() == ArgumentType.INVOCATION;
			boolean rightArgNeedsParantheses = operatorInvocation
					.getRightArgument().getArgType() == ArgumentType.INVOCATION;
			...
			if (rightArgNeedsParantheses) {
				rightArgNeedsParantheses = !(operatorInvocation
						.getRightArgument().getInvocation().get() instanceof NotInvocation);
			}
			...
			if (!operatorInvocation.isArrayAccessOperator()
					&& !operatorInvocation.isUnaryOperator()) {
				cb.append(" ");
			}
			...
			if (!operatorInvocation.isUnaryOperator()) {
			...
			boolean argNeedsParantheses = notInvocation.getArgument()
					.getArgType() == ArgumentType.INVOCATION;
			cb.append("!");
			...
			if (!"this".equals(i.getVariableName())
				...
				cb.append(i.getVariableName()).append(".");
			}
			...
			if (s instanceof SimpleForDeclaration) {
				...
				cb.append("for(").append("int ").append(forD.getVarName())
						.append(" = " + forD.getFrom()).append("; ")
						.append(forD.getVarName());
				...
				Utils.renderComments(Scope2Code.renderedComments, whileD
						.getControlFlow().getInvocations(), whileD, cb, (
						CodeEntity ce) -> {
				...
				Utils.renderComments(Scope2Code.renderedComments, ifD
						.getControlFlow().getInvocations(), ifD, cb, (
						CodeEntity ce) -> {
				...
				Utils.renderComments(Scope2Code.renderedComments, elseD
						.getControlFlow().getInvocations(), elseD, cb, (
						CodeEntity ce) -> {
	...
	private void renderArgument(IArgument arg, CodeBuilder cb) {
		...
		if (arg.getArgType() == ArgumentType.CONSTANT) {
			...
			if (arg.getType().equals(Type.STRING)) {
				constString = "\""
						+ VLangUtilsNew.addEscapeCharsToCode(arg.getConstant()
								.get().getValue(Object.class).toString())
						+ "\"";
			} else {
				constString = String.valueOf(arg.getConstant().get()
...
class MethodDeclarationRenderer implements CodeRenderer<MethodDeclaration> {
	...
	public void render(MethodDeclaration e, CodeBuilder cb) {
		...
		// }
		//
		Utils.renderComments(Scope2Code.renderedComments, e.getControlFlow()
				.getInvocations(), e, cb, (CodeEntity ce) -> {
...
class ClassDeclarationRenderer implements CodeRenderer<ClassDeclaration> {
	...
	public void render(ClassDeclaration cd, CodeBuilder cb) {
		...
		Utils.renderComments(Scope2Code.renderedComments, cd
				.getDeclaredMethods(), cd, cb, (CodeEntity ce) -> {
	...
	private void createDeclaredVariables(ClassDeclaration cd, CodeBuilder cb) {
		for (Variable v : cd.getVariables()) {
			if (!"this".equals(v.getName())) {
				...
				cb.append(v.getType().getClassNameAsCode()).append(" ")
						.append(v.getName()).append(";").newLine();
			}
		}
		cb.newLine();
	}
...
class CompilationUnitRenderer implements
	...
	public void render(CompilationUnitDeclaration e, CodeBuilder cb) {
		...
		Utils.renderComments(Scope2Code.renderedComments, entities, e, cb, (
				CodeEntity ce) -> {
	...
	/**
	 * @param classDeclarationRenderer
	 *            the classDeclarationRenderer to set
	 */
