 ...
 */
public class Heuristic implements eu.mihosoft.ai.astar.Heuristic<CEList> {

    public Heuristic() {
    }

    @Override
    public double estimate(State<CEList> state, Condition<CEList> goal, WorldDescription<CEList> wd) {
        CEList currentList = state.get(0);

        if (!(goal instanceof CEListGoal)) {
            return (int) Double.MAX_VALUE;
        }

        CEList goalList = ((CEListGoal) goal).getCodeEntityList();
