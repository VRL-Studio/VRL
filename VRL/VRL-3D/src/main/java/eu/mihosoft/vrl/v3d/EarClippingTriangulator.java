...
class EarClippingTriangulator {
	...
	private final IntArray indicesArray = new IntArray();
	private int[] indices;
	...
	public IntArray computeTriangles (double[] vertices) {
	...
	private boolean isEarTip (int earTipIndex) {
		...
		double[] vertices = this.vertices;
		double p1x = vertices[p1], p1y = vertices[p1 + 1];
		double p2x = vertices[p2], p2y = vertices[p2 + 1];
		double p3x = vertices[p3], p3y = vertices[p3 + 1];
	...
	static private boolean areVerticesClockwise (double[] vertices, int offset, int count) {
		if (count <= 2) return false;
		double area = 0, p1x, p1y, p2x, p2y;
	...
	static private int computeSpannedAreaSign (double p1x, double p1y, double p2x, double p2y, double p3x, double p3y) {
